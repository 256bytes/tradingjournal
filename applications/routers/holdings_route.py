...
def holdings_page(): #--------------------------------------------------------------------------------------------------------------------Holdings
    ...
    stock_names = db.session.query(Transactions.script.distinct().label("script")).filter(Transactions.user_id == current_user.id, Transactions.type == "CNC")
    get_each_stocks = [i.script for i in stock_names.all()]
    dict = {}
    for symbol in get_each_stocks:
    ...
    return render_template('holdings.html', dict=dict, holdings=holdings, sell_form=sell_form, add_form=add_form,\
